...
This was the end of the conversation but it stuck with me for quite a while. As a grad student at Georgia Tech in CS, I should not be stumped with this simple question, so I decided to write this blog on "**the simple task of hosting an API**".
...
Joining Google Ads straight out of undergrad was an opportunity that I will forever be grateful for. I had immense growth both professionally and personally. I learnt a great deal of software engineering and how production code is supposed to be shipped that can handle planet-scale. So while I learnt a great deal of **using** planet-scale infra, I was severely lacking in technical abilities of actually **building** a planet-scale infra. [TBA Cluade add something of this flavour] In return for learning a great deal about software engineering practices, writing docs, domain knowledge and intuition for Ads, I didnt get to work on techincal stuff like improving the load balancer or decreasing the boot time latency of [Borg](https://en.wikipedia.org/wiki/Borg_(cluster_manager)) machines.
...
So what I'm gonna do is use my fully decked out Raspberry Pi 5 that has been sitting idle for the past 3 years in my closet. This Pi comes with a cooling fan + 128 GB SD storage + 2.4GHz quad-core Arm Cortex-A76 + 8GB RAM running the Linux based Raspberry Pi OS.

Looks something like this before assembling:

![](https://www.canakit.com/Media/700/2915.jpg)

This is the machine that will be hosting my server.

## Setup

Before running the server, we need to get the machine running first. All we need to do is  
